...
Brutality.getBrutalityById = function (brutalityId, result) {
    sql.query("Select * from brutality where brutalityID = ? ", brutalityId, function (err, res) {             
            if(err) {
...
Brutality.getBrutalityByIds = function (brutalityId1, brutalityId2, result) {
    sql.query("Select * from brutality where brutalityID between ? and ?", [ brutalityId1, brutalityId2 ], function (err, res) {             
            if(err) {
                console.log("error: ", err);
                result(err, null);
            }
...
Brutality.getLast20Brutality = function (result) {
    sql.query("Select * from brutality order by date desc limit 20", function (err, res) {
            if(err) {
                console.log("error: ", err);
                result(null, err);
            }
